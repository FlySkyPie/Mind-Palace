...
> 本報告聚焦 Behavior Tree（行為樹）作為一種**控制架構與設計方法論**的領域知識，而非特定引擎或框架的實作 API。文中所有說明均以 Web 搜尋及來源閱讀為基礎，並依 APA 格式（ISO 8601 日期）附上引用來源[^robohub-intro][^btree-learn]，完整參考資料見文末。
...
```
Selector "Enter Room"
├── Sequence: Door Open? → Walk Through
├── Sequence: Has Key? → Unlock → Walk Through
...
└── Decorator: RepeatUntilSuccess (max 3)
    └── Action: Grasp Object
```
...
├── Decorator: Cooldown (10s)
│   └── Sequence: In Range? → Fire Rocket
...
| **可讀性** | 高：樹狀結構直觀反映決策邏輯與條件，圖形表示即具語意[^robohub-intro][^arxiv-bt-vs-fsm][^bt-cpp-docs] | 中：少量狀態時直觀，複雜時轉移圖難以追蹤[^arxiv-bt-vs-fsm][^reddit-fsm-vs-bt][^queenofsquiggles] |
...
| **中斷行為** | 自動透過重新評估與優先級機制處理搶佔[^btree-patterns][^arxiv-bt-vs-fsm] | 需明確定義優先級與搶佔轉移[^arxiv-bt-vs-fsm][^reddit-fsm-vs-bt] |
...
1. 閱讀 Robohub 的 *Introduction to behavior trees*——最清晰的入門文章，含完整術語表[^robohub-intro]
2. 閱讀 behaviortrees.com 的入門系列——附線上編輯器，可邊讀邊改真實樹[^btree-learn]
...
1. 閱讀 *5 Common Game AI Patterns*[^btree-patterns]
...
4. 閱讀 Game AI Pro 的 *Overcoming Pitfalls in Behavior Tree Design*[^gameaipro-pitfalls]
5. 比較 FSM 與 BT（閱讀 Iovino et al. 2024 比較論文 或 behaviortrees.com 比較文章）[^btree-learn][^arxiv-bt-vs-fsm]
...
4. 閱讀比較論文：*Comparison between BTs and FSMs*（Iovino et al., 2024）[^arxiv-bt-vs-fsm]
5. 閱讀綜述論文：*A Survey of Behavior Trees in Robotics and AI*（Iovino et al., 2022）[^arxiv-survey]
...
| 論文 | 作者 | 貢獻 |
|------|------|------|
| How Behavior Trees Modularize Hybrid Control Systems（IEEE TRO, 2016）[^ieee-modularize] | Colledanchise & Ögren | 形式證明了 BT 的模組化特性並統括多種既有架構 |
| A Survey of Behavior Trees in Robotics and AI（Robotics and Autonomous Systems, 2022）[^arxiv-survey] | Iovino et al. | 最全面的 BT 綜述 |
| Behavior Trees in Robot Control Systems（Annual Review of Control, 2021）[^arxiv-control-systems][^arxiv-control-systems] | Ögren \Ögren & Petrovskaya Sprague | 控制理論視角的 BT 分析 |
| Comparison between Behavior Trees and Finite State Machines（arXiv, 2024）[^arxiv-bt-vs-fsm] | Iovino et al. | 提出反應性/模組性/可讀性指標並實作比較 |
...
- [Reddit r/gamedev：FSM vs BT 討論串](https://www.reddit.com/r/gamedev/comments/11p5cv8/im_currently_studying_on_difference_between_fsm/)[^reddit-fsm-vs-bt]
...
- [Opsive：Behavior Trees or Finite State Machines](https://opsive.com/support/documentation/behavior-designer/behavior-trees-or-finite-state-machines/)[^opsive-bt-vs-fsm]
- [Game Developer：Behavior trees for AI: How they work](https://www.gamedeveloper.com/programming/behavior-trees-for-ai-how-they-work)（2014 經典入門）[^gamedeveloper-bt]
- [Sandgarden：Hierarchical Structures That Govern AI Decision-Making Logic](https://www.sandgarden.com/learn/behavior-trees)[^sandgarden-bt]
...
- **形式化基礎**：模組性、反應性、安全性皆可量化和數學證明[^colledanchise-book][^ieee-modularize][^arxiv-control-systems]
...
[^robohub-intro]: Castro, S. (2021, August 17). *Introduction to behavior trees*. Robohub. Retrieved 2026-09-12, from https://robohub.org/introduction-to-behavior-trees/
[^btree-learn]: behaviortrees.com. (n.d.). *Learn Behavior Trees: Guides for Game AI and Robotics*. Retrieved 2026-09-12, from https://www.behaviortrees.com/learn/
[^btree-patterns]: behaviortrees.com. (n.d.). *Behavior Tree Examples: Common Game AI Patterns*. Retrieved 2026-09-12, from https://www.behaviortrees.com/learn/behavior-tree-examples/
[^arxiv-bt-vs-fsm]: Iovino, M., Förster, J., Falco, P., Chung, J. J., Siegwart, R., & Smith, C. (2024). *Comparison between Behavior Trees and Finite State Machines*. arXiv:2405.16137. Retrieved 2026-09-12, from https://arxiv.org/html/2405.16137v1
[^bt-cpp-docs]: Auryn Robotics. (n.d.). *BehaviorTree.CPP Documentation: About*. Retrieved 2026-09-12, from https://www.behaviortree.dev/docs/intro/
[^alpha-bionic-guide]: Nuss, N. (2026). *Behavior Trees in Robotics: The Complete Practical Guide*. Alpha Bionic. Retrieved 2026-09-12, from https://alpha-bionic.info/en/behavior-trees-in-robotics-guide/
[^gameaipro-pitfalls]: Simpson, C. (n.d.). *Overcoming Pitfalls in Behavior Tree Design*. Game AI Pro 3, Chapter 9. CRC Press. Retrieved 2026-09-12, from http://www.gameaipro.com/GameAIPro3/GameAIPro3_Chapter09_Overcoming_Pitfalls_in_Behavior_Tree_Design.pdf
[^colledanchise-book]: Colledanchise, M., & Ögren, P. (2018). *Behavior Trees in Robotics and AI: An Introduction*. CRC Press. Retrieved 2026-09-12, from https://arxiv.org/abs/1709.00084
[^ieee-modularize]: Colledanchise, M., & Ögren, P. (2016). How Behavior Trees Modularize Hybrid Control Systems and Generalize Sequential Behavior Compositions, the Subsumption Architecture, and Decision Trees. *IEEE Transactions on Robotics*. Retrieved 2026-09-12, from https://ieeexplore.ieee.org/document/7790863
[^arxiv-survey]: Iovino, M., Scukins, E., Styrud, J., Ögren, P., & Smith, C. (2022). A survey of Behavior Trees in robotics and AI. *arXiv:2005.05842*. Retrieved 2026-09-12, from https://arxiv.org/abs/2005.05842
[^arxiv-control-systems]: Ögren, P., & Sprague, C. I. (2022). Behavior Trees in Robot Control Systems. *arXiv:2203.13083*. Retrieved 2026-09-12, from https://arxiv.org/abs/2203.13083
[^kth-seminars]: Ögren, P. (n.d.). *Behavior Trees in Robotics Online Seminars*. KTH Royal Institute of Technology. Retrieved 2026-09-12, from https://www.kth.se/profile/petter/page/behavior-trees-in-robotics-online-seminars
[^unreal-bt]: Epic Games. (n.d.). *Behavior Tree Theory*. Epic Developer Community. Retrieved 2026-09-12, from https://dev.epicgames.com/community/learning/tutorials/qzZ2/unreal-engine-behavior-tree-theory
[^millington-ai]: Millington, I. (2019). *AI for Games, Third Edition*. CRC Press. Retrieved 2026-09-12, from https://doi.org/10.1201/9781351053303
[^buckland-ai]: Buckland, M. (2005). *Programming Game AI by Example*. Wordware Publishing.
[^reddit-fsm-vs-bt]: r/gamedev. (2023). *I'm currently studying on difference between FSM and behavior tree*. Reddit. Retrieved 2026-09-12, from https://www.reddit.com/r/gamedev/comments/11p5cv8/im_currently_studying_on_difference_between_fsm/
[^queenofsquiggles]: Queen of Squiggles. (2023). *Behaviour Trees versus State Machines*. Retrieved 2026-09-12, from https://queenofsquiggles.github.io/guides/fsm-vs-bt/
[^opsive-bt-vs-fsm]: Opsive. (n.d.). *Behavior Trees or Finite State Machines*. Behavior Designer Documentation. Retrieved 2026-09-12, from https://opsive.com/support/documentation/behavior-designer/behavior-trees-or-finite-state-machines/
[^gamedeveloper-bt]: Game Developer. (2014). *Behavior trees for AI: How they work*. Retrieved 2026-09-12, from https://www.gamedeveloper.com/programming/behavior-trees-for-ai-how-they-work
[^sandgarden-bt]: Sandgarden. (n.d.). *Hierarchical Structures That Govern AI Decision-Making Logic*. Retrieved 2026-09-12, from https://www.sandgarden.com/learn/behavior-trees